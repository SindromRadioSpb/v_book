...
def copy_project(source_db: Path, target_db: Path, project_name: str = "Hebrew Wikipedia Baseline"):
    ...
    try:
        ...
        if target_cur.fetchone():
            ...
            print(f"[OK] Deleted existing project")
        ...
        target_cur.execute("SELECT library_id FROM library WHERE library_id = ?", (source_library_id,))
        ...
        target_cur.execute("SELECT corpus_id FROM source_corpus WHERE project_id = ? ORDER BY corpus_id", (new_project_id,))
        new_corpus_ids = [row[0] for row in target_cur.fetchall()]

        target_cur.execute("SELECT corpus_id FROM source_db.source_corpus WHERE project_id = ? ORDER BY corpus_id", (source_project_id,))
        ...
        for old_corpus_id, new_corpus_id in corpus_id_map.items():
            ...
            print(f"  - Copying {corpus_doc_count:,} documents from corpus {old_corpus_id} to {new_corpus_id}...")
