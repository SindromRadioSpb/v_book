...
def eval(output, reference):
    ...
    unlabeled_ref = set(
        filter(filt, replace_head_idx_with_form(list(map(unlabeled_func, reference))))
    )
    ...
    return (
        ...
        missing_head,
        missing_label,
        len(list(filter(filt_unmapped, reference))),
    )
...
if __name__ == "__main__":
    ...
    f1_unlabeled = (
        2 * (unlabeled_precision * unlabeled_recall) / (unlabeled_precision + unlabeled_recall)
    )
