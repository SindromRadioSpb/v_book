...
class TMEntryDeduplicator:
    ...
    def _dedupe_lemmas(self):
        ...
        stmt = (
            select(TMEntry.project_id, TMEntry.src_text, func.count(TMEntry.tm_id).label("count"))
    ...
    def _merge_duplicate_group(self, kind: str, project_id: int, src_text: str, count: int):
        ...
        stmt = (
            select(TMEntry)
            .where(
                TMEntry.project_id == project_id, TMEntry.kind == kind, TMEntry.src_text == src_text
            )
            .order_by(
        ...
        logger.info(
            f"    Canonical: tm_id={canonical.tm_id}, origin={canonical.origin}, "
        ...
        for other in others:
            logger.info(
                f"    Merging: tm_id={other.tm_id}, origin={other.origin}, "
        ...
        if kind == "lemma" and not canonical.lemma_id:
            lemma_stmt = select(Lemma).where(
                Lemma.project_id == project_id, Lemma.lemma_text == src_text
            )
            ...
            cluster_stmt = select(TermCluster).where(
                TermCluster.project_id == project_id, TermCluster.representative_he == src_text
            )
        ...
        if canonical.src_norm != normalized.norm:
            logger.info(
                f"      -> Re-normalizing src_norm: '{canonical.src_norm}' -> '{normalized.norm}'"
            )
            if not self.dry_run:
