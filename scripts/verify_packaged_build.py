    ...
    with db_service.get_session() as session:
        ...
        if result:
            ...
            columns = session.execute(
                text("PRAGMA table_info(security_audit_log)")
            ).fetchall()
            column_names = [c[1] for c in columns]

            required_columns = ["log_id", "event_timestamp", "event_type", "outcome",
                              "operation", "resource_type", "resource_id", "reason"]
            ...
            if missing:
                ...
                print(f"  OK security_audit_log has all required columns ({len(column_names)} total)")
        else:
        ...
        if result:
            ...
            required_columns = ["credential_id", "key", "encrypted_value",
            ...
            if missing:
                ...
                print(f"  OK credentials table has all required columns ({len(column_names)} total)")
        else:
