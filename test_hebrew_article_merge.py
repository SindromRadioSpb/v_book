...
def test_merge_standalone_articles():
    ...
    print(f"✅ Test 2: ['ה', 'תנועה', 'הגדולה'] → ['התנועה', 'הגדולה']")
    ...
    print(f"✅ Test 3: ['ה', 'ספר', 'ה', 'גדול'] → ['הספר', 'הגדול']")
    ...
    print(f"✅ Test 5: ['סעיף', 'ה', '.'] → unchanged (enumeration)")
    ...
    assert len(result) == 3, f"Expected 3 tokens, got {len(result)}"
    assert result[1]['text'] == "ה", f"Expected standalone 'ה', got '{result[1]['text']}'"
    print(f"✅ Test 6: ['סעיף', 'ה', ':'] → unchanged (enumeration)")
    ...
    assert len(result) == 3, f"Expected 3 tokens, got {len(result)}"
    assert result[1]['text'] == "ה", f"Expected standalone 'ה', got '{result[1]['text']}'"
    print(f"✅ Test 7: ['סעיף', 'ה', ')'] → unchanged (enumeration)")
    ...
    print(f"✅ Test 8: ['ה', '=', '5'] → unchanged (variable)")
    ...
    assert len(result) == 3, f"Expected 3 tokens, got {len(result)}"
    assert result[0]['text'] == "ה", f"Expected standalone 'ה', got '{result[0]['text']}'"
    print(f"✅ Test 9: ['ה', '=', 'k'] → unchanged (variable)")
    ...
    assert len(result) == 3, f"Expected 3 tokens, got {len(result)}"
    assert result[1]['text'] == "ה", f"Expected standalone 'ה', got '{result[1]['text']}'"
    print(f"✅ Test 10: ['ספר', 'ה', ','] → unchanged (punctuation)")
    ...
    print(f"✅ Test 13: ['ספר', 'גדול'] → unchanged (no prefixes)")
    ...
    try:
        with db_service.get_session() as session:
            ...
            project = project_service.create_project(
                session,
                "Article Merge Test",
                "Test standalone article merging"
            )
            ...
            print(f"\n{'='*60}")
            print(f"✅ INTEGRATION TEST PASSED: Article merging works!")
            print(f"{'='*60}")
            return True
