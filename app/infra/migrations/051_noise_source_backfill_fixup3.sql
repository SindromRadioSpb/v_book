...
-- Migration 051: Third pass noise_source backfill.
--
-- Migration 050 was applied to some databases via the old db.py exception handler
-- ('duplicate column name' guard) which set schema_version=50 WITHOUT executing
-- the UPDATE statements. This left up to 287 tm_entry rows with noise_source=NULL.
-- This migration re-runs the identical backfill idempotently (WHERE noise_source IS NULL).
-- Safe on already-correct databases — all WHERE clauses are no-ops.
