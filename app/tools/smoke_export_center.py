...
class ExportTestCase:
    ...
    def __init__(
        self, format_name: str, file_name: str, export_func, validator_func, options: dict = None
    ):
...
class SmokeRunner:
    ...
    def setup(self):
        ...
        with self.db_service.get_session() as session:
            ...
            if not matching:
                ...
                raise ValueError(
                    f"Project '{self.project_name}' not found. " f"Available projects: {available}"
                )
    ...
    def run_test_case(self, test_case: ExportTestCase):
        ...
        try:
            ...
            logger.info(
                f"  ✅ PASS: {test_case.file_size} bytes, {test_case.entries_count} entries"
            )
    ...
    def validate_csv(self, test_case: ExportTestCase):
        """Validate CSV export."""
        with open(test_case.path, encoding="utf-8", newline="") as f:
            reader = csv.reader(f)
            ...
            if test_case.entries_count == 0:
                self.coverage_warnings.append("CSV export: 0 TM entries (empty data)")

    def validate_jsonl(self, test_case: ExportTestCase):
        """Validate JSON export (array format, not true JSON Lines)."""
        with open(test_case.path, encoding="utf-8") as f:
            content = f.read()

            if not content.strip():
                if test_case.entries_count == 0:
                    self.coverage_warnings.append("JSON export: 0 entries (empty data)")
                    return
                else:
    ...
    def validate_xlsx(self, test_case: ExportTestCase):
        ...
        # Entries can be 0 (smoke test coverage warning)
        if test_case.entries_count == 0:
            self.coverage_warnings.append("XLSX export: 0 dictionary entries (empty data)")
    ...
    def validate_tbx(self, test_case: ExportTestCase):
        ...
        if test_case.entries_count == 0:
            ...
            if approved_only:
                ...
                self.coverage_warnings.append("TBX export: 0 term entries (empty data)")
        ...
        if len(term_entries) > 0:
            ...
            langs = set(
                ls.get("{http://www.w3.org/XML/1998/namespace}lang") or ls.get("xml:lang")
                for ls in langsets
            )
    ...
    def validate_tmx(self, test_case: ExportTestCase):
        ...
        if test_case.entries_count == 0:
            ...
            if not include_draft:
                ...
                self.coverage_warnings.append("TMX export: 0 translation units (empty data)")
        ...
        try:
            ...
            with self.db_service.get_session() as session:
                ...
                if len(ordered_specs) != 17:
                    ...
                    logger.info("[OK] Catalog has 17 specs")
    ...
    def run_all(self):
        ...
        for i, test_case in enumerate(self.test_cases, 1):
            logger.info(
                f"[{i}/{len(self.test_cases)}] {test_case.format_name}: {test_case.file_name}"
            )
        ...
        for tc in self.test_cases:
            ...
            options_str = (
                ", ".join(f"{k}={v}" for k, v in tc.options.items()) if tc.options else "default"
            )
            logger.info(
        ...
        if self.coverage_warnings:
            ...
            logger.info(
                "Note: Empty data warnings are expected for smoke tests on sparse projects."
            )
...
Examples:
  ...
  python -m app.tools.smoke_export_center --db /path/to/hdle.db --keep-artifacts
    ...
    parser.add_argument(
        "--project-name", default="ТЕСТ М8,М9", help="Project name to test (default: ТЕСТ М8,М9)"
    )
    parser.add_argument(
        "--outdir",
    ...
    parser.add_argument(
        "--keep-artifacts", action="store_true", help="Keep export artifacts after test"
    )
