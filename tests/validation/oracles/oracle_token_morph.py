...
Comparison strategy:
- token_count: exact integer match
...
- morph_contains: partial feature match — each listed feature string must appear
  as a substring of Token.morph. This is intentionally loose to survive minor
  Stanza version differences in the full feature string.
- morph: exact full string match — only checked if the gold case provides it
  (used in C02 implementation wave when stricter pinning is warranted).
