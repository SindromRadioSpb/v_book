...
def reference_project(db_service, project_service):
    ...
    with db_service.get_session() as session:
        ...
        project = session.get(project_service.db_service.sa_models.DictProject, project_id)
...
def test_cannot_add_documents_to_reference_corpus(
    ...
    with db_service.get_session() as session:
        ...
        if not corpus:
            corpus_id = project_service.create_corpus(
                session, reference_project, "Test Corpus"
            )
        else:
            corpus_id = corpus.corpus_id

        with pytest.raises(ReferenceCorpusReadonlyError, match="Cannot add documents to reference corpus"):
            ingest_service.import_file(session, corpus_id, test_file)
...
def test_cannot_delete_documents_from_reference_corpus(
    ...
    with db_service.get_session() as session:
        project = session.get(project_service.db_service.sa_models.DictProject, reference_project)
        ...
        doc_id = ingest_service.import_file(session, corpus_id, test_file)
        session.commit()
        ...
        with pytest.raises(ReferenceCorpusReadonlyError, match="Cannot delete documents from reference corpus"):
            ingest_service.delete_document(session, doc_id)
...
def test_can_modify_non_reference_project(db_service, project_service, ingest_service, tmp_path):
    ...
    with db_service.get_session() as session:
        ...
        corpus_id = project_service.create_corpus(session, project_id, "Test Corpus")
