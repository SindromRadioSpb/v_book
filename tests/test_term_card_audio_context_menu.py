...
def _build_term_card_view(selected_count: int):
    ...
    state = {"generate": 0, "play": 0, "playlist": 0, "add": 0, "edit_pron": 0, "bootstrap": 0}
...
def test_term_card_context_menu_includes_audio_actions(monkeypatch):
    ...
    assert FakeMenu.last.actions[2].text == "Add Selected to Playlist (3 rows)..."
    assert FakeMenu.last.actions[3].text == "Add Selected to User Dictionary (3 rows)..."
    assert FakeMenu.last.actions[4].text == "Mispronounced -> Add Pronunciation..."
    assert FakeMenu.last.actions[5].text == "Pronunciation Bootstrap Selected (3 rows)..."
